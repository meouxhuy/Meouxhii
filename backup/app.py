...
async def fetch_html_and_extract_links(session_http, video_data, semaphore, cancel_flag=None):
    ...
    async with semaphore:
        # Kiểm tra hủy ngay khi lấy được semaphore
        if cancel_flag and cancel_flag.is_set():
        ...
        try:
            # Thêm delay ngẫu nhiên để tránh bị phát hiện (0.1-0.3 giây)
            await asyncio.sleep(0.1 + (hash(vid) % 20) / 100)
            
            current_type = video_data['type']
            html_content = ""
            
            # OPTIMIZATION: Only attempt shorts URL if video is short candidate (<=185s)
            if is_short_candidate:
                ...
                # Skip shorts check for long videos, go straight to watch URL
                if current_type != 'Stream':
                    video_data['type'] = 'Video'
                video_data['url'] = f"https://www.youtube.com/watch?v={vid}"
            ...
            # FIX: Robust text-based Premiere detection to prevent misclassification as Stream
            if video_data['type'] == 'Stream':
                # Remove all spaces from HTML for more aggressive matching
                html_no_spaces = html_content.replace(' ', '')
                
                # Check multiple Premiere indicators (UI text + JSON tags)
                is_premiere = (
                ...
                # Only reclassify as Video if it's a Premiere AND NOT a real live stream
                if is_premiere and not is_real_live:
                    video_data['type'] = 'Video'
            
            # OPTIMIZATION: Offload CPU-bound regex parsing to thread pool to prevent event loop blocking
...
async def process_all_urls(urls, start_date, end_date, cancel_flag=None):
    candidate_ids =[]
    final_channel_name = "MeoU"
    
    # Danh sách User-Agent để rotate (giả lập nhiều trình duyệt khác nhau)
    user_agents = [
    ...
    # Add TCPConnector with limit=100 for better concurrent connection handling
    connector = aiohttp.TCPConnector(limit=100)
    async with aiohttp.ClientSession(headers=headers, connector=connector) as session_http:
        for u in urls:
            # Kiểm tra hủy trước mỗi kênh
            if cancel_flag and cancel_flag.is_set():
                break
        ...
        for i in range(0, len(unique_ids), 50):
            # Kiểm tra hủy trước mỗi batch video metadata
            if cancel_flag and cancel_flag.is_set():
                break
                ...
                async with session_http.get(api_url) as resp:
                    ...
                    for item in data.get('items',[]):
                        ...
                        if start_date <= pub_date <= end_date:
                            ...
                            # Parse duration to determine if video is a short candidate (<=185s)
        ...
        tasks =[fetch_html_and_extract_links(session_http, v, semaphore, cancel_flag) for v in valid_videos]
...
def scan_links():
    ...
    # Tạo cancel flag mới cho lần scan này
    ...
    try:
        ...
        scanned_results, final_channel_name = loop.run_until_complete(
            process_all_urls(data.get('urls',[]), data.get('startDate'), data.get('endDate'), cancel_flag)
        )
        loop.close()
    except Exception as e:
        print(f"Lỗi Scan: {e}")
        return jsonify({"results":[], "channel_name": "Lỗi", "cancelled": False})
    finally:
        SCAN_CANCEL_FLAGS.pop(user, None)

    if cancel_flag.is_set():
        return jsonify({"results": [], "channel_name": "Đã hủy", "cancelled": True})
        
    scanned_results.sort(key=lambda x: x['upload_date'], reverse=True)
    return jsonify({"results": scanned_results, "channel_name": final_channel_name, "cancelled": False})
